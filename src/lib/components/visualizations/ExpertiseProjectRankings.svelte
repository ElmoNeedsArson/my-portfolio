<script lang="ts">
    ...
    import { getProjectPath } from "$lib/projectTabs";
        ...
        {:else}
            {#each rankedProjects as ranked, index}
                <div 
                    ...
                    on:click={() => navigateToProject(ranked.project)}
                    on:keydown={(e) => e.key === 'Enter' && navigateToProject(ranked.project)}
                    role="button"
                    tabindex="0"
                >
                    <!-- Rank badge -->
