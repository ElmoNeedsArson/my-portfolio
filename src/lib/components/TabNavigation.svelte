<script lang="ts">
  ...
  import { ToyBrick, Pin, FileText } from "@lucide/svelte";
  import { goto } from "$app/navigation";
  import { page } from "$app/state";
  import { onMount, tick } from "svelte";

  const iconMap = { ToyBrick, Pin, FileText };
  ...
  let currentTabId = $derived(getTabIdByPath(page.url.pathname) ?? DEFAULT_TAB_ID);

  function switchTab(id: TabId) {
    goto(getTabPathById(id));
  }
