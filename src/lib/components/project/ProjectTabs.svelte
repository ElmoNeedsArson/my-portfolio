<script lang="ts">
    ...
    import { getProjectTabs } from "$lib/projectTabs";
    import type { ProjectTabDefinition } from "$lib/projectTabs";
    ...
    let tabs: ProjectTabDefinition[] = [];
    let activeTabIndex = 0;

    $: tabs = getProjectTabs(project);

    $: if (tabs.length > 0) {
        const idx = initialTabId ? tabs.findIndex((t) => t.slug === initialTabId) : -1;
