<script lang="ts">
  ...
  // Extract sections from the project content
  $: sections = extractSections(project);

  function extractSections(project: Project): OutlineSection[] {
    const sections: OutlineSection[] = [];

    // Add overview if it exists
    if (project.content.overview?.trim()) {
      sections.push({
        id: "overview",
        title: "Overview",
        level: 1,
    ...
    project.content.sections.forEach((section, index) => {
