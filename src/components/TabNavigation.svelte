<script lang="ts">
  ...
  onMount(() => {
    ...
    return () => {
      ...
      ro.disconnect();
      if (runningAnim) try { runningAnim.cancel(); } catch {}
    };
  });
</script>
...
.tab { background: transparent; border: none; color: var(--muted-color); padding: 1rem 1.5rem; cursor: pointer; font: inherit; font-size: 1.5rem; font-weight:500; transition: color .3s ease; display:flex; align-items:center; justify-content:center; gap:.75rem; letter-spacing:.1rem; position:relative; z-index:1; text-align:center; }
...
.tab-label { font-weight:500; letter-spacing:.1rem; }
...
@media (max-width:768px){ .tab-container { grid-template-columns:1fr; } .tab { justify-content:flex-start; padding:.75rem .75rem .75rem .5rem; font-size:1.3rem; } }
@media (max-width:480px){ .tab { font-size:1.1rem; } }
</style>
